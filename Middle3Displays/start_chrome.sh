...
while [ 1 ]
do
DISPLAY=:0 /usr/bin/google-chrome-stable  -–incognito --new-window --disable-session-crashed-bubble  --disable-infobars --window-position=0,${offset} --window-size=1900,1150 --no-default-browser-check --user-data-dir=${HOME}/.google-chrome/session0.${screen} ${url} 
done
